...
class TestExtractionBehavior:
    """验证 NeuroMemory 在 message_interval=1 时每条消息都触发提取"""

    async def test_extraction_triggered_on_every_message(self):
        """每条 add_message 都应触发 _do_extraction"""
        from neuromemory import NeuroMemory, ExtractionStrategy
        ...
        for i in range(5):
            # Replicate NeuroMemory._on_message_added counting logic
    ...
    async def test_reflection_triggered_after_20_extractions(self):
        ...
        for i in range(25):
            # Simulate extraction count tracking (from _do_extraction)
            extract_counts[user_id] = extract_counts.get(user_id, 0) + 1
            if extraction.reflection_interval > 0:
                if extract_counts[user_id] >= extraction.reflection_interval:
...
class TestMainAppConfig:
    """验证 main.py 中 NeuroMemory 初始化参数"""
    ...
    async def test_extraction_strategy_params(self):
        ...
        assert strategy.reflection_interval == 20
